    ...
    if mode == 2:
        return [
            UIBoxSpec(
                ...
                border_style="heavy",
                title="Aviso",
                body_text="Faisca Magica is now attuned to Water affinity.",
                x=3,
                y=13,
                max_body_width=42,
                body_align="center",
...
def _actor_action_options(actor_key: str, summon_hawking_unlocked: bool = False) -> List[str]:
    ...
    if key == "player":
        ...
        return options
    if key == "mushy":
        return ["Atacar", "Cha de Cogumelo", "Defender"]
    if key == "sharoom":
        return ["Atacar", "Toque de Cura", "Defender"]
    if key == "roomy":
...
def _action_is_available(flow: dict, actor_key: str, action: str) -> bool:
    ...
    if key == "player" and act == "Invocar Hawking":
...
def _spell_targeting_meta(actor_key: str, action: str) -> dict | None:
    ...
    if a == "mushy" and act == "Cha de Cogumelo":
        return {
            ...
            "supports_all": False,
            "default_mode": "single",
            "desc": "Cha de Cogumelo (2 MP): +3 Atacar e +3 Defesa. Acumula. Reduz 1 a cada turno.",
        }
    ...
    if a == "roomy" and act == "Concentrico":
        return {
            ...
            "supports_all": False,
            "default_mode": "all",
            "desc": "Concentrico (2 MP): restaura +1 MP para cada aliado, exceto o conjurador.",
        }
    return None
...
def _alive_secondary_target_indices(flow: dict, actor_key: str, action: str, stage: int) -> List[int]:
    ...
    if a == "mushy" and act == "Cha de Cogumelo":
        # Cha de Cogumelo can target any living teammate, including the caster.
        return alive
    if a == "roomy" and act == "Concentrico":
...
def _build_screen_spec(flow: dict) -> UIBoxSpec | None:
    ...
    if screen == "start_confirm":
        return UIBoxSpec(
            ...
            body_text=(
                ...
                f"Nome: {flow.get('selected_name', 'WARRIOR')}\n"
                f"Fortuna: {flow.get('fortune_choice', 'Bem de Vida (100 GP)')}\n\n"
                "Iniciar esta configuracao de novo jogo?"
            ),
            center_x=50,
            center_y=17,
            max_body_width=52,
    ...
    if screen == "story_5":
        return UIBoxSpec(
            ...
            title="Mushy",
            body_text=(
                "Eu tenho um Cajado Magico com o feitico Faisca Magica. "
                "Se voce descobrir como usar, podemos acabar com esse corvo juntos."
            ),
            center_x=50,
            center_y=17,
            max_body_width=44,
    ...
    if screen == "story_battle_cmd_player":
        ...
        return UIBoxSpec(
            role="battle_select",
            border_style="double",
            title=str(flow.get("selected_name", flow.get("avatar_label", "Jogador"))),
            body_text="\n" + "\n".join(lines),
            center_x=50,
            center_y=17,
            max_body_width=32,
    ...
    if screen == "story_hawk_intro_1":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Hawking",
            body_text="Eu sou o Rei dos Falc?es. Finalmente encontrei voces, boboes que espancam passaros.",
            center_x=50,
            center_y=17,
            max_body_width=48,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_post_3":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Hawking",
            body_text="So tente nao chamar toda hora. Talvez uma vez por batalha esteja bom por enquanto.",
            center_x=50,
            center_y=17,
            max_body_width=48,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_victory":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Sistema",
            body_text="Pena de Hawking adquirida. Invocar Hawking agora esta disponivel para o Jogador.",
            center_x=50,
            center_y=17,
            max_body_width=54,
            wrap_mode="balanced",
    ...
    if screen == "story_mp_increase":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Recompensa",
            body_text="Seu nivel de magia aumentou. MP maximo +2.",
            center_x=50,
            center_y=17,
            max_body_width=42,
            wrap_mode="balanced",
    ...
    if screen == "story_sharoom_1":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Seus Pontos de Mana estao aumentando. Isso significa que voce pode usar MP para lan?ar feiti?os em batalha quando os itens ficarem sem cargas.",
            center_x=50,
            center_y=17,
            max_body_width=52,
            wrap_mode="balanced",
    ...
    if screen == "story_sharoom_5":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Sharoom",
            body_text="Me avise quando precisar de Toque de Cura, essa e minha especialidade.",
            center_x=50,
            center_y=17,
            max_body_width=44,
            wrap_mode="balanced",
    ...
    if screen == "story_more_crows":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Seu Mycostaff tem 3 cargas de Faisca Magica. Ele recarrega automaticamente apos cada batalha.",
            center_x=50,
            center_y=17,
            max_body_width=48,
            wrap_mode="balanced",
    ...
    if screen == "story_more_crows_2":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Tambem podemos usar meu Cha de Cogumelo para aumentar temporariamente Atacar e Defesa, ganhando vantagem na batalha.",
            center_x=50,
            center_y=17,
            max_body_width=50,
            wrap_mode="balanced",
...
def _build_battle_round_actions(flow: dict) -> List[dict]:
    ...
    for actor_key, actor_idx, target_key in turn_order:
        ...
        if actor_key == "player" and action == "Faisca Magica":
            ...
            continue
        if actor_key == "player" and action == "Invocar Hawking":
...
def _battle_action_log_lines(action: dict, stage: int, player_name: str) -> List[str]:
    ...
    if kind == "spell":
        ...
        lines = [f"{src} conjura {spell_name} em {dst} e causa {damage} de dano."]
    elif kind == "physical":
        lines = [f"{src} ataca {dst} e causa {damage} de dano."]
    elif kind == "defend":
        ...
        lines = [f"{src} usa Cha de Cogumelo em {dst}, aumentando Atacar e Defesa."]
        ...
        lines = [f"{src} usa Toque de Cura em toda a equipe."]
    elif kind == "concentric":
        lines = [f"{src} usa Concentrico para restaurar MP da equipe."]
    elif kind == "birdcall":
        lines = [f"{src} usa Chamado das Aves. Um Corvo Bebe entrou na luta."]
    elif kind == "flee":
        lines = [f"{src} foge da batalha."]
    elif kind == "summon":
        lines = [f"{src} invoca ataque da Pena de Hawking em {dst} e causa {damage} de dano."]
    else:
        lines = [f"{src} age."]
...
def _battle_log_start(flow: dict, stage: int) -> None:
    ...
    flow["battle_round"] = 1
    if int(stage) >= 5:
        opener = "A batalha comeca: Sharoom, Jogador, Mushy e Roomy vs 3 Fadas Bebe."
    elif int(stage) >= 4:
...
def render(
    ...
    # Voltarground sky pass: drifting cloud sprites.
    for cloud in clouds:
    ...
    # Voltarground ground pass: 15 rows of grass and pebbles.
...
def main() -> None:
    ...
    flow = {
        ...
        "battle_staff_charges": 3,          # Mycostaff charges for Faisca Magica
                ...
                if screen == "root_menu":
                    ...
                    if key == "up":
                        ...
                        if cursor == 0:
                            ...
                            flow["message_text"] = "Menu Jogo Salvo selecionado. (Demo)"
                            begin_transition("info")
                        else:
                            flow["message_text"] = "Explorador de Recursos selecionado. (Demo)"
                            begin_transition("info")
                    ...
                    if key == "up":
                        ...
                        cmd_idx = _next_enabled_option_index(enabled, cmd_idx, 1)
                        flow["battle_player_cmd_idx"] = cmd_idx
                    elif key == "left" and enabled[cmd_idx] and options[cmd_idx] in ("Atacar", "Faisca Magica", "Invocar Hawking"):
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, -1)
                    elif key == "right" and enabled[cmd_idx] and options[cmd_idx] in ("Atacar", "Faisca Magica", "Invocar Hawking"):
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, 1)
                    elif confirm:
