...
def layout_actor_strip(area: LayoutZone, sprites: List[List[List[str]]], spacing: int = 1) -> List[dict]:
    ...
    x = start_x
    for rows, (w, h) in zip(active, sizes):
        y = area.y1 - max(0, h - 1)
...
def render(
    ...
    if world_layer_level > 0:
        ...
        for actor in layout_actor_strip(secondary_zone, sec_sprites, spacing=1):
    ...
    if world_layer_level >= 2 and primary_zone is not None:
        ...
        for actor in layout_actor_strip(primary_zone, pri_sprites, spacing=1):
            x0 = int(actor.get("x", 0))
            y0 = int(actor.get("y", 0))
            rows = actor.get("rows", [])
            if not isinstance(rows, list):
...
def main() -> None:
    ...
    beba_sprite = build_opponent_sprite(opponents, "fairy_baby", color_codes)
    ...
    try:
        ...
        while True:
            ...
            frame = render(
                ...
                world_anchor_stagger=world_anchor_stagger,
                world_treeline_sprites=world_treeline_sprites,
                primary_actor_sprites=[mushy_sprite, beba_sprite],
                secondary_actor_sprites=[guy_sprite, chase_sprite],
