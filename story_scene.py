...
def build_forest_band(
    ...
    rows: List[List[str]] = [[] for _ in range(pano.content_height)]

    for _ in range(6):
        tree_id = tree_options[rng.randrange(len(tree_options))]
        pano._append_piece(rows, pano._object_art(tree_id), pano._object_mask(tree_id))
    ...
    if mush_art:
        pano._append_piece(rows, mush_art, mush_mask)
