...
def _build_screen_spec(flow: dict) -> UIBoxSpec | None:
    ...
    if screen in ("story_2", "story_8_cast", "story_10_cast", "story_10_melt"):
        return None
...
def render(
    ...
    if primary_placements and secondary_placements:
        ...
        if isinstance(story_spell, dict):
            s_idx = int(story_spell.get("source_secondary_index", 0))
            t_idx = int(story_spell.get("target_primary_index", 0))
...
def main() -> None:
    ...
    try:
        ...
        while True:
            ...
            if anim_mode == "open" and story_action == "cast1":
            ...
            elif anim_mode == "open" and story_action == "cast2":
                flow["story_action_t"] = float(flow.get("story_action_t", 0.0)) + dt
                if float(flow["story_action_t"]) >= 1.2:
                    flow["crow_hp"] = 0
                    flow["player_mp"] = 2
            ...
            elif anim_mode == "open" and story_action == "melt":
                flow["story_action_t"] = float(flow.get("story_action_t", 0.0)) + dt
                if float(flow["story_action_t"]) >= 1.0:
                    flow["story_action"] = None
                    flow["story_action_t"] = 0.0
            ...
            if anim_mode == "open" and key is not None and flow.get("story_action") is None:
                if screen == "root_menu":
                    ...
                    if confirm:
                        begin_transition("story_7")
                elif screen == "story_7":
                    if confirm:
                        flow["screen"] = "story_8_cast"
                        flow["story_action"] = "cast1"
                        flow["story_action_t"] = 0.0
                elif screen == "story_9":
                    if confirm:
                        flow["screen"] = "story_10_cast"
                        flow["story_action"] = "cast2"
                        flow["story_action_t"] = 0.0
                elif screen == "story_11":
                    if confirm:
                        target_sky_rows = 25
                        begin_transition("root_menu")
