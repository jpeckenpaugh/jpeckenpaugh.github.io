...
def _actor_action_options(actor_key: str, summon_hawking_unlocked: bool = False) -> List[str]:
    ...
    if key == "player":
        options = ["Attack", "Chispa Magica"]
        if summon_hawking_unlocked:
            options.append("Invocar a Hawking")
        options.append("Defend")
        return options
    if key == "mushy":
        return ["Attack", "Te de Hongo", "Defend"]
    if key == "sharoom":
        return ["Attack", "Toque Sanador", "Defend"]
    if key == "roomy":
...
def _action_is_available(flow: dict, actor_key: str, action: str) -> bool:
    ...
    if act in ("Te de Hongo", "Toque Sanador", "Concentric"):
        return mp >= 2
    return True
...
def _build_screen_spec(flow: dict) -> UIBoxSpec | None:
    ...
    if screen == "start_confirm":
        return UIBoxSpec(
            ...
            body_text=(
                ...
                f"Fortune: {flow.get('fortune_choice', 'Comodo (100 GP)')}\n\n"
    ...
    if screen == "story_mp_increase":
        ...
        body = "Tu nivel de magia aumento. MP max +2."
        if reward_stage == 2:
            body += "\n\nEl hechizo Chispa Magica subio a Nivel 2. Ahora el hechizo apunta a multiples oponentes."
        return UIBoxSpec(
            role="story",
            border_style="heavy",
            title="Recompensa",
        ...
        while True:
                ...
                if melt_index is not None:
                    ...
                    if qidx >= len(queue):
                        ...
                        if action_t >= duration:
                            ...
                            stage_now = int(flow.get("battle_stage", 1))
                            player_name = str(flow.get("selected_name", "Jugador")).strip() or "Jugador"
            ...
            if anim_mode == "open" and key is not None and flow.get("story_action") is None:
                if screen == "root_menu":
                    ...
                    if key == "up":
                        ...
                        cmd_idx = _next_enabled_option_index(enabled, cmd_idx, 1)
                        flow["battle_player_cmd_idx"] = cmd_idx
                    elif key == "left" and enabled[cmd_idx] and options[cmd_idx] in ("Attack", "Invocar a Hawking"):
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, -1)
                    elif key == "right" and enabled[cmd_idx] and options[cmd_idx] in ("Attack", "Invocar a Hawking"):
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, 1)
                    elif key == "left" and enabled[cmd_idx] and options[cmd_idx] == "Chispa Magica" and magic_spark_level < 2:
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, -1)
                    elif key == "right" and enabled[cmd_idx] and options[cmd_idx] == "Chispa Magica" and magic_spark_level < 2:
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, 1)
                    elif confirm:
                        ...
                        if pick in ("Attack", "Chispa Magica", "Invocar a Hawking"):
            ...
            if screen in ("story_battle_cmd_player", "story_battle_cmd_mushy", "story_battle_cmd_sharoom", "story_battle_cmd_roomy"):
                if screen == "story_battle_cmd_player":
                    ...
                    magic_spark_level = max(1, int(flow.get("battle_magic_spark_level", 1)))
                    if pick == "Chispa Magica" and magic_spark_level >= 2:
