...
DEFAULT_SKY_ROWS = 15
UI_DEMO_TEXT = "Eeny, meeny, miny, mo.\nQuien aqui se atreve a ser nuestro rival!?"
UI_DIALOG_TEXT = "Entonces que dices... Estas listo para desafiarlos?"
DEMO_BATTLE_LOG_LINES = [
    "Mushy lanza Chispa Magica sobre 4 Guerreras Hada.",
    "Beba lanza Chispa Magica sobre 4 Guerreras Hada.",
    "Una Guerrera Hada ha sido derrotada.",
    "Una Guerrera Hada ha sido derrotada.",
    "Hada 1 lanza Luz Curativa sobre Hada 3.",
]
...
def build_ui_demo_specs(variant: int) -> List[UIBoxSpec]:
    ...
    if mode == 1:
        return [
            UIBoxSpec(
                ...
                border_style="double",
                title="Notificacion",
                body_text="Estados resueltos. Orden de turnos actualizado.",
                x=2,
                y=12,
                max_body_width=44,
                body_align="left",
            ...
            UIBoxSpec(
                ...
                border_style="double",
                title="Beba",
                body_text="Entonces que dices... Estas listo para desafiarlos?",
                center_x=54,
                center_y=18,
                max_body_width=54,
                wrap_mode="balanced",
    ...
    if mode == 2:
        return [
            UIBoxSpec(
                ...
                border_style="heavy",
                title="Aviso",
                body_text="Chispa Magica ahora esta alineada con afinidad de Agua.",
                x=3,
                y=13,
                max_body_width=42,
                body_align="center",
            ...
            UIBoxSpec(
                role="prompt",
                border_style="light",
                title="Aviso de Accion",
                body_text="Usar esta configuracion para el siguiente encuentro?",
                center_x=50,
                center_y=18,
                max_body_width=46,
                wrap_mode="balanced",
    ...
    return [
        UIBoxSpec(
            ...
            border_style="light",
            title="Notificacion",
            body_text="Progreso de mision actualizado: quedan 2 objetivos.",
            x=2,
            y=12,
            max_body_width=44,
        ),
...
def build_ui_animation_specs() -> List[UIBoxSpec]:
    return [
        UIBoxSpec(
            ...
            border_style="light",
            title="Notificacion",
            body_text="Progreso de mision actualizado.",
            center_x=17,
            center_y=17,
            max_body_width=22,
            body_align="center",
        ...
        UIBoxSpec(
            ...
            border_style="heavy",
            title="Aviso",
            body_text="Iniciar esta batalla ahora?",
            center_x=50,
            center_y=17,
            max_body_width=24,
            body_align="center",
        ...
        UIBoxSpec(
            ...
            border_style="double",
            title="Historial",
            body_text="Mushy lanza Chispa Magica.\nGuerrera Hada derrotada.",
            center_x=83,
            center_y=17,
            max_body_width=22,
            body_align="left",
...
def build_new_game_workflow_specs() -> List[UIBoxSpec]:
    return [
        UIBoxSpec(
            ...
            body_align="left",
            wrap_mode="normal",
            actions=["[ A / Confirmar ]", "[ S / Atras ]"],
            anchor="center",
        ),
        UIBoxSpec(
            role="avatar_select",
            border_style="double",
            title="Elegir Aventurero",
            body_text=(
                "Slot 1\n"
                "\n"
                "[ GUY ]                 GAL\n"
        ...
        UIBoxSpec(
            ...
            body_text=(
                ...
                "MAYUS  LISTO  CANCELAR\n"
                "\n"
                "Flechas mueven. A selecciona. S cancela."
            ),
            center_x=50,
            center_y=17,
            max_body_width=62,
        ...
        UIBoxSpec(
            ...
            body_text=(
                "Avatar: Guy\n"
                "Nombre: MUSHY\n"
                "Fortuna: Comodo (100 GP)\n"
                "\n"
                "Iniciar nuevo juego con esta configuracion?"
            ),
            center_x=50,
            center_y=17,
            max_body_width=52,
...
def _apply_name_key(name: str, key_token: str, shift: bool) -> tuple[str, bool, bool, bool]:
    ...
    if token == "SPACE":
        ...
        return (name, shift, False, False)
    if token in ("DONE", "LISTO"):
        if name.strip():
            return (name, shift, True, False)
        return (name, shift, False, False)
    if token in ("CANCEL", "CANCELAR"):
...
def _actor_action_options(actor_key: str, summon_hawking_unlocked: bool = False) -> List[str]:
    ...
    if key == "player":
        options = ["Atacar", "Chispa Magica"]
        if summon_hawking_unlocked:
            options.append("Invocar a Hawking")
        options.append("Defender")
        return options
    if key == "mushy":
        return ["Atacar", "Te de Hongo", "Defender"]
    if key == "sharoom":
        return ["Atacar", "Toque Sanador", "Defender"]
    if key == "roomy":
        return ["Atacar", "Concentrico", "Defender"]
...
def _spell_targeting_meta(actor_key: str, action: str) -> dict | None:
    ...
    act = str(action).strip()
    if a == "mushy" and act == "Te de Hongo":
        return {
            "side": "ally",
            "supports_all": False,
            "default_mode": "single",
            "desc": "Te de Hongo (2 MP): +3 Atacar and +3 Defense. Stacks. Decays by 1 each turn.",
        }
    if a == "sharoom" and act == "Toque Sanador":
        return {
            "side": "ally",
            "supports_all": True,
            "default_mode": "single",
            "desc": "Toque Sanador (2 MP): objetivo individual cura hasta 5 HP, o TODOS los aliados curan hasta 2 HP cada uno.",
        }
    if a == "roomy" and act == "Concentrico":
        return {
            ...
            "supports_all": False,
            "default_mode": "all",
            "desc": "Concentrico (2 MP): restaura +1 MP a cada aliado excepto el lanzador.",
        }
    return None
...
def _alive_secondary_target_indices(flow: dict, actor_key: str, action: str, stage: int) -> List[int]:
    ...
    if a == "mushy" and act == "Te de Hongo":
        # Te de Hongo can target any living teammate, including the caster.
        return alive
    if a == "roomy" and act == "Concentrico":
        # Concentrico excludes the caster.
...
def _build_screen_spec(flow: dict) -> UIBoxSpec | None:
    ...
    if screen == "root_menu":
        options = ["Nuevo Juego", "Juego Guardado", "Explorador de Recursos"]
        ...
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Menu Principal",
            body_text="\n".join(lines + ["", "Usa Arriba/Abajo y A/S."]),
            center_x=50,
            center_y=17,
            max_body_width=34,
            actions=["[ A / Confirmar ]", "[ S / Atras ]"],
    ...
    if screen == "avatar_select":
        label = str(flow.get("avatar_label", "Aventurero"))
        return UIBoxSpec(
            role="avatar_select",
            border_style="double",
            title="Elegir Aventurero",
            body_text=f"Slot 1  Avatar: {label}\n\nUsa Izquierda/Derecha para cambiar.",
            center_x=50,
            center_y=17,
            max_body_width=62,
            actions=["[ A / Confirmar ]", "[ S / Atras ]"],
    ...
    if screen == "name_input":
        ...
        return UIBoxSpec(
            ...
            border_style="double",
            title="Nombre Personalizado",
            body_text="\n".join(lines + ["", "Flechas mueven. A selecciona tecla."]),
            center_x=50,
            center_y=17,
            max_body_width=72,
            body_align="left",
    ...
    if screen == "fortune_select":
        options = ["Pobre (10 GP)", "Comodo (100 GP)", "Realeza (1000 GP)"]
        cursor = int(flow.get("fortune_cursor", 1)) % len(options)
        lines = _format_select_lines(options, cursor)
        return UIBoxSpec(
            role="fortune_select",
            border_style="heavy",
            title="Elegir Fortuna",
            body_text="\n".join(lines + ["", "Elige tu oro inicial."]),
            center_x=50,
            center_y=17,
            max_body_width=42,
            body_align="left",
    ...
    if screen == "start_confirm":
        return UIBoxSpec(
            ...
            title="Comenzar Aventura",
            body_text=(
                f"Avatar: {flow.get('avatar_label', 'Aventurero')}\n"
                f"Nombre: {flow.get('selected_name', 'WARRIOR')}\n"
                f"Fortuna: {flow.get('fortune_choice', 'Comodo (100 GP)')}\n\n"
                "Iniciar esta configuracion de nuevo juego?"
            ),
            center_x=50,
            center_y=17,
            max_body_width=52,
    ...
    if screen == "story_1":
        return UIBoxSpec(
            ...
            title="Historia",
            body_text=(
                "Un dia, mientras regresas a casa, escuchas un alboroto. "
                "Te encuentras con un pequeno hongo magico y un cuervo que parecen estar peleando."
            ),
            center_x=50,
            center_y=17,
            max_body_width=52,
    ...
    if screen == "story_4":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Oye! Pareces una buena persona. Vendrias a ayudarme con este cuervo molesto?",
            center_x=50,
            center_y=17,
            max_body_width=42,
            wrap_mode="balanced",
    ...
    if screen == "story_5":
        return UIBoxSpec(
            ...
            title="Mushy",
            body_text=(
                "Tengo un Basto Magico con el hechizo Chispa Magica. "
                "Si logras usarlo, podemos deshacernos de este cuervo juntos."
            ),
            center_x=50,
            center_y=17,
            max_body_width=44,
    ...
    if screen == "story_6":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Genial! Ahora mostremosle a este cuervo quien manda aqui.",
            center_x=50,
            center_y=17,
            max_body_width=40,
            wrap_mode="balanced",
    ...
    if screen == "story_battle_cmd_player":
        ...
        return UIBoxSpec(
            role="battle_select",
            border_style="double",
            title=str(flow.get("selected_name", flow.get("avatar_label", "Jugador"))),
            body_text="\n" + "\n".join(lines),
            center_x=50,
            center_y=17,
            max_body_width=32,
    ...
    if screen == "story_battle_ally_target":
        ...
        action = str(flow.get("battle_ally_action", "Te de Hongo"))
        ...
        mode_line = f"Modo: {'TODOS' if mode == 'all' else 'Individual'}"
        hint = "I/D objetivo"
        if supports_all:
            hint += "  U/D individual/todos"
    ...
    if screen == "story_battle_victory":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Ahora deberiamos buscar a Roomie. Creo que esta buscando un lugar para quedarse...",
            center_x=50,
            center_y=17,
            max_body_width=44,
            wrap_mode="balanced",
    ...
    if screen == "story_roomy_2":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Roomie! Por aqui. Necesitamos tu apoyo en esta guerra contra los cuervos.",
            center_x=50,
            center_y=17,
            max_body_width=44,
            wrap_mode="balanced",
    ...
    if screen == "story_roomy_3":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Roomie",
            body_text="He estado trabajando en mi hechizo Concentrico.",
            center_x=50,
            center_y=17,
            max_body_width=42,
            wrap_mode="balanced",
    ...
    if screen == "story_roomy_3b":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Roomie",
            body_text="Es un hechizo para recuperar MP, y puede ser util en batallas largas, cuando necesitamos puntos de magia extra.",
            center_x=50,
            center_y=17,
            max_body_width=50,
            wrap_mode="balanced",
    ...
    if screen == "story_roomy_4":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Un hechizo para recuperar puntos magicos, dices?",
            center_x=50,
            center_y=17,
            max_body_width=42,
            wrap_mode="balanced",
    ...
    if screen == "story_roomy_4b":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Si lo combinamos con nuestros otros hechizos, podemos aumentar mucho nuestro poder.",
            center_x=50,
            center_y=17,
            max_body_width=50,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_intro_1":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Hawking",
            body_text="Soy el Rey de los Halcones. Por fin encontre a esos tontos rompe-pajaros.",
            center_x=50,
            center_y=17,
            max_body_width=48,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_intro_2":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Hawking",
            body_text="Por que no intentan meterse con alguien de su tamano?",
            center_x=50,
            center_y=17,
            max_body_width=44,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_intro_3":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Sharoom",
            body_text="De nuestro tamano? Debes ser al menos 10 veces mas grande que nosotros.",
            center_x=50,
            center_y=17,
            max_body_width=48,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_intro_4":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Sharoom",
            body_text="Pero nosotros tenemos trabajo en equipo, y tu estas solo!",
            center_x=50,
            center_y=17,
            max_body_width=44,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_intro_5":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Hawking",
            body_text="Ah, si creen eso?",
            center_x=50,
            center_y=17,
            max_body_width=40,
            wrap_mode="balanced",
            body_align="left",
            actions=["[ A / Iniciar Batalla ]"],
        )
    ...
    if screen == "story_hawk_post_1":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Hawking",
            body_text="Bien, ganaron. Supongo que podemos ser amigos, ya que al parecer no son debiles.",
            center_x=50,
            center_y=17,
            max_body_width=52,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_post_2":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Hawking",
            body_text="Toma, quedate con una de mis plumas. Solo llamame cuando necesites ayuda y caere en picada.",
            center_x=50,
            center_y=17,
            max_body_width=50,
            wrap_mode="balanced",
    ...
    if screen == "story_hawk_post_3":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Hawking",
            body_text="Solo intenta no llamarme tan seguido. Tal vez una vez por batalla este bien por ahora.",
            center_x=50,
            center_y=17,
            max_body_width=48,
            wrap_mode="balanced",
            body_align="left",
            actions=["[ A / Aceptar Pluma de Hawking ]"],
        )
    ...
    if screen == "story_hawk_victory":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Sistema",
            body_text="Pluma de Hawking obtenida. Invocar a Hawking ahora esta disponible para el Jugador.",
            center_x=50,
            center_y=17,
            max_body_width=54,
            wrap_mode="balanced",
    ...
    if screen == "story_post_hawk_fairy_intro":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Roomy",
            body_text="Uy, ahi vienen esas hadas...",
            center_x=50,
            center_y=17,
            max_body_width=40,
            wrap_mode="balanced",
    ...
    if screen == "story_second_chance":
        return UIBoxSpec(
            ...
            body_text="Una luz desciende del cielo, y la esperanza regresa.",
            center_x=50,
            center_y=17,
            max_body_width=50,
            wrap_mode="balanced",
    ...
    if screen == "story_mp_increase":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Recompensa",
            body_text="Tu nivel de magia aumento. MP max +2.",
            center_x=50,
            center_y=17,
            max_body_width=42,
            wrap_mode="balanced",
    ...
    if screen == "story_sharoom_1":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Tus Puntos de Mana estan aumentando. Eso significa que puedes usar MP para lanzar hechizos en batalla cuando tus objetos se queden sin cargas.",
            center_x=50,
            center_y=17,
            max_body_width=52,
            wrap_mode="balanced",
    ...
    if screen == "story_sharoom_2":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Tengo una amiga, Sharoom, que conoce magia de curacion. Seria una gran adicion para nuestro pequeno equipo.",
            center_x=50,
            center_y=17,
            max_body_width=50,
            wrap_mode="balanced",
    ...
    if screen == "story_sharoom_3":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Sharoom, ha llegado la hora! La guerra contra los cuervos ya comenzo. Ven a unirte a nuestra lucha...",
            center_x=50,
            center_y=17,
            max_body_width=44,
            wrap_mode="balanced",
    ...
    if screen == "story_sharoom_4":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Sharoom",
            body_text="Claro, me unire a su equipo. Creo que podemos trabajar muy bien juntos.",
            center_x=50,
            center_y=17,
            max_body_width=44,
            wrap_mode="balanced",
    ...
    if screen == "story_sharoom_5":
        return UIBoxSpec(
            ...
            border_style="double",
            title="Sharoom",
            body_text="Avisame cuando necesites un Toque Sanador, esa es mi especialidad.",
            center_x=50,
            center_y=17,
            max_body_width=44,
            wrap_mode="balanced",
    ...
    if screen == "story_more_crows":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Tu Micobaston guarda 3 cargas del hechizo Chispa Magica. Se recarga automaticamente despues de cada batalla.",
            center_x=50,
            center_y=17,
            max_body_width=48,
            wrap_mode="balanced",
    ...
    if screen == "story_more_crows_2":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Tambien podemos usar mi Te de Hongo para aumentar temporalmente nuestros puntos de Ataque y Defensa, y tener ventaja en batalla.",
            center_x=50,
            center_y=17,
            max_body_width=50,
            wrap_mode="balanced",
    ...
    if screen == "story_more_crows_3":
        return UIBoxSpec(
            ...
            border_style="heavy",
            title="Mushy",
            body_text="Cuidado, vienen mas cuervos!",
            center_x=50,
            center_y=17,
            max_body_width=42,
            wrap_mode="balanced",
    ...
    return UIBoxSpec(
        ...
        border_style="light",
        title="Aviso",
        body_text=message or "Marcador de demo.",
        center_x=50,
        center_y=17,
        max_body_width=50,
        body_align="center",
        wrap_mode="balanced",
        actions=["[ A / OK ]", "[ S / Atras ]"],
    )
...
def _position_screen_box_for_actors(
    ...
    if screen in ("story_roomy_4", "story_roomy_4b") and len(secondary_placements) >= 2:
        idx = 2 if len(secondary_placements) >= 3 else 1
        return _anchor_box_next_to_actor(spec, secondary_placements[idx], prefer="right")
    if screen in ("story_hawk_intro_1", "story_hawk_intro_2", "story_hawk_intro_5", "story_hawk_post_1", "story_hawk_post_2", "story_hawk_post_3") and primary_placements:
        return _anchor_box_next_to_actor(spec, primary_placements[0], prefer="left")
    ...
    if screen in ("story_battle_victory", "story_more_crows") and len(secondary_placements) >= 2:
        return _anchor_box_next_to_actor(spec, secondary_placements[1], prefer="right")
    if screen in ("story_hawk_victory", "story_post_hawk_fairy_intro") and len(secondary_placements) >= 2:
...
def _build_battle_round_actions(flow: dict) -> List[dict]:
    ...
    pri_kind = [str(v).strip().lower() for v in flow.get("battle_primary_kind", (["baby_crow", "hawk", "baby_crow"] if stage >= 4 else []))]
    ...
    for i in range(len(pri_hp)):
        kind = pri_kind[i] if i < len(pri_kind) else "baby_crow"
        if stage >= 4 and kind == "hawk":
            pri_base_atk.append(4)
            pri_base_def.append(1)
            pri_base_luck.append(3)
        else:
            pri_base_atk.append(3)
            pri_base_def.append(0)
            pri_base_luck.append(2)
    queue: List[dict] = []
    ...
    for actor_key, actor_idx, target_key in turn_order:
        ...
        if action == "Atacar":
            ...
            continue
        if actor_key == "player" and action == "Chispa Magica":
            ...
            continue
        if actor_key == "player" and action == "Invocar a Hawking":
            ...
            pri_hp[target] = post_hp
            continue
        if actor_key == "mushy" and action == "Te de Hongo":
            if actor_idx >= len(sec_mp) or sec_mp[actor_idx] < 2:
                _enqueue_physical_from_secondary(actor_idx, target_key)
                continue
            allies = _alive_secondary_target_indices(flow, "mushy", "Te de Hongo", stage)
            if not allies:
                allies = [actor_idx]
            ...
            continue
        if actor_key == "sharoom" and action == "Toque Sanador":
            if actor_idx >= len(sec_mp) or sec_mp[actor_idx] < 2:
                _enqueue_physical_from_secondary(actor_idx, target_key)
                continue
            mode = str(flow.get("battle_sharoom_spell_target_mode", "single")).strip().lower()
            valid_targets = _alive_secondary_target_indices(flow, "sharoom", "Toque Sanador", stage)
            pre_mp = sec_mp[actor_idx]
            post_mp = max(0, pre_mp - 2)
            sec_mp[actor_idx] = post_mp
...
def _battle_action_log_lines(action: dict, stage: int, player_name: str) -> List[str]:
    ...
    if kind == "spell":
        spell_name = str(action.get("spell_name", "Chispa Magica")).strip() or "Chispa Magica"
        lines = [f"{src} lanza {spell_name} sobre {dst} y causa {damage} de dano."]
    elif kind == "physical":
        lines = [f"{src} ataca a {dst} y causa {damage} de dano."]
    elif kind == "defend":
        lines = [f"{src} adopta una postura defensiva."]
    elif kind == "defend_convert":
        stat = str(action.get("convert_stat", "HP"))
        lines = [f"{src} convierte defender en +1 {stat}."]
    elif kind == "mushroom_tea":
        lines = [f"{src} usa Te de Hongo en {dst}, aumentando Ataque y Defensa."]
    elif kind == "healing_touch_single":
        heal_amount = max(0, int(action.get("heal_amount", 0)))
        lines = [f"{src} usa Toque Sanador en {dst}, restaurando {heal_amount} HP."]
    elif kind == "healing_touch_team":
        lines = [f"{src} usa Toque Sanador en todo el equipo."]
    elif kind == "concentric":
        lines = [f"{src} usa Concentrico para restaurar MP del equipo."]
    elif kind == "birdcall":
        lines = [f"{src} usa Llamado de Aves. Un Cuervo Bebe se une a la batalla."]
    elif kind == "flee":
        lines = [f"{src} huye de la batalla."]
    elif kind == "summon":
        lines = [f"{src} invoca un golpe de Pluma de Hawking sobre {dst} y causa {damage} de dano."]
    else:
        lines = [f"{src} acts."]
    if pre_hp > 0 and post_hp <= 0 and kind not in ("flee", "birdcall"):
        if bool(action.get("surrender_on_zero", False)):
            lines.append(f"{dst} se rinde.")
        else:
            lines.append(f"{dst} ha sido derrotado.")
    return lines
...
def _battle_log_start(flow: dict, stage: int) -> None:
    ...
    flow["battle_round"] = 1
    if int(stage) >= 5:
        opener = "Comienza la batalla: Sharoom, Jugador, Mushy y Roomy vs 3 Hadas Bebe."
    elif int(stage) >= 4:
        opener = "Comienza batalla mini-jefe: Sharoom, Jugador, Mushy y Roomy vs Hawking."
    elif int(stage) >= 3:
        opener = "Comienza la batalla: Sharoom, Jugador y Mushy vs 3 Cuervos Bebe."
    elif int(stage) == 2:
        opener = "Comienza la batalla: Jugador y Mushy vs 2 Cuervos Bebe."
    else:
        opener = "Comienza la batalla: Jugador y Mushy vs Cuervo Bebe."
...
def _draw_hawking_swoop(
    canvas: List[List[str]],
    hawk_rows: List[List[str]],
    target_actor: dict,
    progress: float,
) -> None:
    if not isinstance(hawk_rows, list) or not hawk_rows:
        return
    ...
    th = len(t_rows)
    hw = max((len(r) for r in hawk_rows), default=0)
    hh = len(hawk_rows)
    tx = int(target_actor.get("x", 0)) + (tw // 2) - (hw // 2)
    ty = int(target_actor.get("y", 0)) + (th // 2) - (hh // 2)
    p = max(0.0, min(1.0, float(progress)))
    sx = SCREEN_W + 4
    sy = -max(2, hh + 2)
    t = min(1.0, p / 0.72)
    ...
    if 0.72 <= p < 0.84:
        # One blink while overlapping the target before impact.
        blink_on = (int((p - 0.72) / 0.06) % 2) == 0
        if not blink_on:
            return
        x, y = tx, ty
    elif p >= 0.84:
...
def render(
    ...
    # Atrasground sky pass: drifting cloud sprites.
    for cloud in clouds:
    ...
    # Atrasground ground pass: 15 rows of grass and pebbles.
    ...
    # Historia battle overlays: targeting, projectile, HP, and defeat cues.
    if primary_placements and secondary_placements:
        ...
        if isinstance(story_primary_hp, list):
            totals = story_primary_hp_totals if isinstance(story_primary_hp_totals, list) else []
            total_default = max(1, int(story_primary_hp_total))
            ...
            for idx, hp_val in enumerate(story_primary_hp):
                ...
                if idx in hidden_idx or int(hp_val) <= 0:
                    continue
                total = max(1, int(totals[idx] if idx < len(totals) else total_default))
...
def main() -> None:
    ...
    fairy_baby_sprite = build_opponent_sprite(opponents, "fairy_baby", color_codes)
    ...
    flow = {
        ...
        "battle_staff_charges": 3,          # Mycostaff charges for Chispa Magica
    ...
    def _compute_sharoom_shift_positions(player_sprite: List[List[str]]) -> tuple[Dict[str, dict], Dict[str, dict]]:
        ...
        for idx, crow in enumerate(end_pri):
            ...
            rows = crow["rows"]
            h = len(rows) if isinstance(rows, list) else 0
            # Iniciar flighted crows above viewport; end in primary battle slots.
        ...
        while True:
            ...
            if anim_mode == "open" and screen in ("story_battle_resolve", "story_battle3_resolve"):
                ...
                if melt_index is not None:
                    ...
                    if qidx >= len(queue):
                        ...
                        hawking_surrendered = stage_now == 4 and hawk_i >= 0 and hawk_i < len(pri_hp) and int(pri_hp[hawk_i]) <= 0
                ...
                if screen == "root_menu":
                    ...
                    if key == "up":
                        ...
                        if cursor == 0:
                            ...
                            flow["message_text"] = "Menu de Juego Guardado seleccionado. (Demo)"
                            begin_transition("info")
                        else:
                            flow["message_text"] = "Explorador de Recursos seleccionado. (Demo)"
                            begin_transition("info")
                    ...
                    if confirm:
                        ...
                        flow["battle_primary_kind"] = ["baby_crow"]
                    ...
                    if key == "up":
                        ...
                        cmd_idx = _next_enabled_option_index(enabled, cmd_idx, 1)
                        flow["battle_player_cmd_idx"] = cmd_idx
                    elif key == "left" and enabled[cmd_idx] and options[cmd_idx] in ("Atacar", "Chispa Magica", "Invocar a Hawking"):
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, -1)
                    elif key == "right" and enabled[cmd_idx] and options[cmd_idx] in ("Atacar", "Chispa Magica", "Invocar a Hawking"):
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, 1)
                    elif confirm:
                        ...
                        if pick in ("Atacar", "Chispa Magica", "Invocar a Hawking"):
                    ...
                    if key == "up":
                        ...
                        cmd_idx = _next_enabled_option_index(enabled, cmd_idx, 1)
                        flow["battle_mushy_cmd_idx"] = cmd_idx
                    elif key == "left" and enabled[cmd_idx] and options[cmd_idx] == "Atacar":
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, -1)
                    elif key == "right" and enabled[cmd_idx] and options[cmd_idx] == "Atacar":
                        flow["battle_target_cursor"] = _next_alive_index(pri_hp, cursor, 1)
                    elif confirm:
                        if not enabled[cmd_idx]:
                            continue
                    ...
                    if confirm:
                        begin_transition("story_hawk_post_3")
                elif screen == "story_hawk_post_3":
                    if confirm:
                    ...
                    if confirm:
                        ...
                        flow["battle_primary_kind"] = ["fairy_baby", "fairy_baby", "fairy_baby"]
                        ...
                        flow["battle_melt_index"] = None
                        flow["battle_melt_t"] = 0.0
                        _battle_log_start(flow, 5)
                        _reset_battle_command_picks(flow, 5)
                        begin_transition("story_battle_cmd_player")
            ...
            if scene_world_layer_level >= 3:
                if screen in battle_screens:
                    ...
                    stage_now = int(flow.get("battle_stage", 1))
                    if stage_now >= 5:
                        primary_sprites = [fairy_baby_sprite for _ in range(enemy_count)]
                    elif stage_now >= 4:
                        kinds = [str(v).strip().lower() for v in flow.get("battle_primary_kind", ["baby_crow", "hawk", "baby_crow"])]
                        if len(kinds) < enemy_count:
                            kinds.extend(["baby_crow"] * (enemy_count - len(kinds)))
                        primary_sprites = [(hawk_sprite if kinds[i] == "hawk" else crow_sprite) for i in range(enemy_count)]
                    else:
                    ...
                    secondary_sprites = [sharoom_sprite, selected_player_sprite, mushy_sprite, roomie_sprite]
                elif screen in ("story_hawk_post_1", "story_hawk_post_2", "story_hawk_post_3"):
                    primary_sprites = [hawk_sprite]
                    secondary_sprites = [sharoom_sprite, selected_player_sprite, mushy_sprite, roomie_sprite]
                elif screen == "story_hawk_victory":
                    primary_sprites = [hawk_sprite]
            ...
            story_primary_hp_totals = list(primary_hp_max_vals)
            if screen in ("story_battle_cmd_player", "story_battle_cmd_mushy", "story_battle_cmd_sharoom", "story_battle_cmd_roomy"):
            ...
            if screen in ("story_battle_resolve", "story_battle3_resolve"):
                ...
                if qidx < len(queue):
                    ...
                    elif kind in ("physical", "summon"):
                        if kind == "physical":
                            smash_prog = prog
                        else:
                            local = max(0.0, min(1.0, (prog - 0.84) / 0.16))
                            smash_prog = 0.5 + (0.5 * local)
                        story_smash = {
                            "source_side": str(action.get("source_side", "secondary")),
                            "source_index": int(action.get("source_index", 0)),
                            "target_side": str(action.get("target_side", "primary")),
            ...
            hawk_keep_visible = screen in ("story_hawk_intro_1", "story_hawk_intro_2", "story_hawk_intro_3", "story_hawk_intro_4", "story_hawk_intro_5", "story_hawk_post_1", "story_hawk_post_2", "story_hawk_post_3", "story_hawk_victory")
            if screen in battle_screens:
