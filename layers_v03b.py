...
def _draw_moon(canvas: List[List[str]], sky_zone: LayoutZone) -> None:
    moon_rows = [
        "   .-ooo-.   ",
        " .*ooooooo*. ",
        ".oooooOooooo.",
    ...
    color = "\x1b[38;2;230;235;255m"
    crater = "\x1b[38;2;185;195;220m"
    width = len(moon_rows[0])
    height = len(moon_rows)
    x0 = SCREEN_W - width - 8
    y0 = max(0, sky_zone.y + 1)
    if y0 + height - 1 > sky_zone.y1:
        y0 = max(sky_zone.y, sky_zone.y1 - height + 1)
    for dy, row in enumerate(moon_rows):
        y = y0 + dy
        ...
        for dx, ch in enumerate(row):
            ...
            x = x0 + dx
            if 0 <= x < SCREEN_W:
                tone = crater if ch == "O" else color
                canvas[y][x] = f"{tone}{ch}{ANSI_RESET}"
...
def render(
    ...
    # Background sky base: full moon behind cloud layer.
    _draw_moon(canvas, sky_zone)
